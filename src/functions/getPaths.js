export function getPostPath(currCategory, currSubcategory, path, pathEdges)
{
    ...
    let category_path = sameCategoryEdges
        .filter(edge => edge.node.frontmatter.sub_category === null)
        .map(edge => edge.node.frontmatter.path)[0]

    let subcategory_path = sameCategoryEdges
        .filter(edge => edge.node.frontmatter.sub_category === currSubcategory)
        .map(edge => edge.node.frontmatter.path)[0]
    ...
    {
      console
        .error("Discrepency between config file and one of markdown files.")
    }
