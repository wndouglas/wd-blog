...
repaying. It turns out that with an initial student debt of £40,000 and an annual
pay rise of 3.6%, you maximise the amount you end up repaying if you have an initial
gross annual salary of £47,250. If this is your salary, you will end up paying
close to an additional 100% of your initial loan in interest charges.
...
Netlfix account, but I would also point out that if you still have an outstanding 
student loan after 30 years, your loan is **written off**. Your 50 year old self 
can then relax in the knowledge that the last trace of your naive student years 
is behind you.
...
3%. At the time of writing, UK RPI is at 2.4%. So if you are still studying right
now, you are paying 5.4% in interest.
...
The next step in our model is to introduce a **traded asset or portfolio** with a 
(hopefully) positive return that we can invest in. We will denote the value of 
...
**systematic risk**, meaning there are always random factors influencing returns.
However, the point of our model is to provide a tractable way to generate 
ballpark figures with which to compare repayment strategies. For now, we avoid 
complications arising from more realistic assumptions in exchange for a more 
tractable model that produces less realistic results, but can still hopefully 
compare the **relative** merits of different approaches. 
...
represents a *set and forget* approach, where a fixed proportion of ones remaining
monthly income is used to make extra voluntary loan repayments.
...
starting salary levels is plotted. The plots show that for starting
